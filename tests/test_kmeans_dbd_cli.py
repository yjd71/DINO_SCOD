...
import select_kmeans_dbd as cli
...
def _build_inputs(
    ...
    boundary = torch.tensor([0.0, 0.95, 0.8, 0.0, 0.9, 0.7], dtype=torch.float32)
    global_disagreement = torch.tensor(
        [0.0, 0.1, 0.2, 0.0, 0.3, 0.4], dtype=torch.float32
    )
    ...
    output_dir = data_root / "splits" / "kmeans_dbd_dedup"
    argv = [
        "--data-root",
        str(data_root),
        "--train-sets",
    ...
    return data_root, output_dir, sample_keys, argv, score_payload


def test_cli_defaults_match_formal_protocol() -> None:
    args = cli.parse_args(
        [
            "--selector-checkpoint",
            "selector.pth",
    ...
    assert args.dino_feature_cache.as_posix().endswith(
...
def test_cli_source_has_no_model_forward_or_score_computation() -> None:
    source = Path(cli.__file__).read_text(encoding="utf-8")
    algorithm = (Path(cli.__file__).parent / "utils" / "kmeans_dbd.py").read_text(
        encoding="utf-8"
    )
    combined = source + algorithm
    for forbidden in (
        "BaseModel",
        "score_pool(",
    ...
    assert "global_disagreement" not in algorithm
    assert "scores" not in algorithm


def test_dry_run_outputs_idempotence_and_conflict(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    _, output_dir, sample_keys, argv, _ = _build_inputs(tmp_path, monkeypatch)

    assert cli.main([*argv, "--dry-run"]) == 0
    report = json.loads(capsys.readouterr().out)
    assert report["catalog_count"] == 6
    assert report["ranking_uses_boundary_disagreement"] is True
    assert report["ranking_uses_global_disagreement"] is False
    ...
    for label in (2, 3, 4, 6):
        artifact_stem = f"kmeans_dbd_dedup_{label:04d}"
        if label == 2:
            artifact_stem += "_seed"
    ...
    with (output_dir / "kmeans_dbd_dedup_assignments.csv").open(
    ...
    assert {"boundary_disagreement", "decision", "dedup_skip_count"} <= set(rows[0])

    manifest = json.loads(
        (output_dir / "kmeans_dbd_dedup_manifest.json").read_text(encoding="utf-8")
    )
    assert manifest["protocol"] == "kmeans_dbd_incluster_dino_dedup_v1"
    assert manifest["selection"]["uses_boundary_disagreement"] is True
    assert manifest["selection"]["uses_global_disagreement"] is False
    assert manifest["selection"]["uses_pc_bacs_value"] is False
    assert manifest["selection"]["dedup_scope"] == (
        "same_cluster_all_previously_selected"
    )
    for label in (2, 3, 4, 6):
        artifact_stem = f"kmeans_dbd_dedup_{label:04d}"
        if label == 2:
        ...
        assert manifest["outputs"][str(label)]["split_fingerprint"] == (
            compute_labeled_split_fingerprint(keys)
        )

    changed = output_dir / "kmeans_dbd_dedup_0003_labeled_names.txt"
    changed.write_text("different\n", encoding="utf-8")
    with pytest.raises(FileExistsError, match="Refusing to overwrite"):
        cli.main(argv)


def test_changing_dall_and_v_does_not_change_dry_run_splits(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    boundary = torch.as_tensor(payload["boundary_disagreement"])
    payload["global_disagreement"] = torch.zeros_like(boundary)
    payload["scores"] = boundary.clone()
    ...
    assert first["first_added"] == second["first_added"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("score", "boundary_disagreement", "half", "dtype"),
    ],
)
def test_cache_mismatches_are_hard_failures(
    ...
    _, output_dir, _, argv, _ = _build_inputs(tmp_path, monkeypatch)
...
def test_selector_seed_fingerprint_mismatch_is_rejected(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _, output_dir, _, argv, _ = _build_inputs(tmp_path, monkeypatch)
