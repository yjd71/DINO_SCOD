...
KMEANS_DALL_DEDUP_PROTOCOL_VERSION = "kmeans_dall_incluster_dino_dedup_v1"
KMeansDAllDirection = Literal["high", "low"]


@dataclass(frozen=True)
class KMeansDAllDecision:
    decision: str
    skip_count: int
...
@dataclass(frozen=True)
class KMeansDAllSelectionResult:
    direction: KMeansDAllDirection
    seed_keys: tuple[str, ...]
    splits: dict[int, list[str]]
    selection_order: tuple[str, ...]
    selection_rank: dict[int, dict[str, int]]
    first_selected_target: dict[str, int]
    decisions: dict[str, KMeansDAllDecision]
    rounds: tuple[KMeansDAllRound, ...]
    dedup_threshold: float
    protocol_version: str = KMEANS_DALL_DEDUP_PROTOCOL_VERSION


def build_kmeans_dall_nested_splits(
    sample_keys: Sequence[str],
    features: Any,
    cluster_ids: Sequence[int] | np.ndarray | torch.Tensor,
    center_distances: Sequence[float] | np.ndarray | torch.Tensor,
    global_disagreement: Sequence[float] | np.ndarray | torch.Tensor,
    seed_keys: Sequence[str],
    *,
    direction: KMeansDAllDirection,
    target_counts: Sequence[int] = (41, 202, 404, 808),
    dedup_threshold: float = 0.95,
) -> KMeansDAllSelectionResult:
    """Select nested KMeans splits ranked directly by global disagreement.

    ``direction='high'`` ranks by ``(-D_all, sample_key)`` and
    ``direction='low'`` ranks by ``(D_all, sample_key)``.  No transformed
    complement is constructed, so the original float32 ordering is retained.
    DINO features are used only for same-cluster cosine deduplication.
    """
    ...
    disagreement = _validated_vector(
        global_disagreement,
        expected_length=len(keys),
    ...
    def rank_key(index: int) -> tuple[float, str]:
        value = float(disagreement[index])
        return (-value, keys[index]) if direction == "high" else (value, keys[index])
    ...
    for target in targets:
        ...
        if len(selected) != target:
            raise RuntimeError(
                f"KMeans-D_all selected {len(selected)} samples for target {target}."
            )
    ...
    decisions = {
        key: KMeansDAllDecision(**mutable_decisions[key]) for key in keys
    }
    return KMeansDAllSelectionResult(
        direction=direction,
